...
def record_http_request(method: str, endpoint: str, status: int, duration_ms: float) -> None:
...
def record_llm_call(
    ...
    llm_tokens_total.labels(service=service, model=model, type="prompt").inc(prompt_tokens)
    llm_tokens_total.labels(service=service, model=model, type="completion").inc(completion_tokens)
    llm_cost_usd_total.labels(service=service, model=model).inc(cost_usd)
    llm_request_duration_seconds.labels(service=service, model=model).observe(duration_ms / 1000)
